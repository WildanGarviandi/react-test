...
export function UpdateFilters(filters) {
  return (dispatch, getState) => {
    const prevFilters = getState().app.myOngoingTrips.filters;
...
export function FetchList() {
  return (dispatch, getState) => {
    ...
    if (filters.userOrderNumber) {
      delete params.limit;
      delete params.offset;
    }

    if (filters.userOrderNumber === '') {
      delete params.userOrderNumber;
    }
    ...
    FetchGet('/trip/ongoing', token, params)
      .then(response => {
        if (!response.ok) {
  ...
  return (dispatch, getState) => {
    ...
    FetchGet('/trip/' + id, token)
      ...
      .catch(e => {
        window.history.back();
        const message =
          e && e.message ? e.message : 'Failed to fetch trip details';
...
export function ExportTrip() {
  return (dispatch, getState) => {
    const { myOngoingTrips, userLogged } = getState().app;
    const { filters, total } = myOngoingTrips;
    const { token } = userLogged;
    const acceptHeader =
    ...
    xhr.onload = function(oEvent) {
      ...
      let fileName =
        'export_' + moment(new Date()).format('YYYY-MM-DD HH:mm:ss') + '.xlsx';
...
export function ReassignDriver(tripID, driverID) {
  return (dispatch, getState) => {
    ...
    dispatch({ type: modalAction.BACKDROP_SHOW });
    FetchDelete(`/trip/${tripID}/driver`, token)
      .then(response => {
...
export function BulkAssignDriver(trips, driverID) {
  return (dispatch, getState) => {
    ...
    dispatch({ type: modalAction.BACKDROP_SHOW });
    FetchDelete(`/trip/bulk-deassign`, token, bodyDeassign)
      .then(response => {
        if (!response.ok) {
          return response.json().then(({ error }) => {
            throw error;
      ...
      .then(response => {
        ...
        response.json().then(function({ data }) {
          dispatch({
            type: Constants.SHOW_SUCCESS_ASSIGN,
            errorIDs:
              (data.failedTripIDs.length > 0 && data.failedTripIDs) || [],
