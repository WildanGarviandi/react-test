...
export function editDriver(id, updateData) {
  return (dispatch, getState) => {
    ...
    dispatch({ type: modalAction.BACKDROP_SHOW });
    FetchPost('/driver/' + id, token, updateData)
      .then(response => {
        if (response.ok) {
  ...
  return (dispatch, getState) => {
    ...
    FetchGet('/driver/' + id, token)
      ...
      .catch(e => {
        window.history.back();
        const message =
          e && e.message ? e.message : 'Failed to fetch driver details';
...
export function addDriver(driverData) {
  return (dispatch, getState) => {
    ...
    dispatch({ type: modalAction.BACKDROP_SHOW });
    FetchPost('/driver', token, driverData)
      .then(response => {
        if (response.ok) {
          response.json().then(function({ data }) {
            dispatch({
          ...
          response.json().then(function({ error }) {
            dispatch(
              ModalActions.addMessage('Failed to add driver: ' + error.message)
            );
            dispatch({ type: modalAction.BACKDROP_HIDE });
          });
        }
...
export function FetchListOrders(id) {
  return (dispatch, getState) => {
    ...
    FetchGet('/driver/' + driverID + '/orders', token, params)
      .then(response => {
        if (!response.ok) {
          return response.json().then(({ error }) => {
            throw error;
      ...
      .catch(e => {
        dispatch(
          ModalActions.addMessage(
            (e && e.message) || 'Cannot connect to server'
          )
        );
      });
  };
