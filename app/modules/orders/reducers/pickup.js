...
export default (state = initialState, action) => {
  switch(action.type) {
    case Constants.PICKUP_ORDERS_FETCH_START: {
      return lodash.assign({}, state, {isFetching: true});
    }

    case Constants.PICKUP_ORDERS_FETCH_END: {
      return lodash.assign({}, state, {isFetching: false});
    }

    case Constants.PICKUP_ORDERS_GROUP_START: {
      return lodash.assign({}, state, {isGrouping: true});
    }

    case Constants.PICKUP_ORDERS_GROUP_END: {
      return lodash.assign({}, state, {isGrouping: false});
    }

    case Constants.PICKUP_ORDERS_SET_FILTER: {
      return lodash.assign({}, state, {
        filter: lodash.assign({}, state.filter, action.filter),
      });
    }

    case Constants.PICKUP_ORDERS_SET_LIST: {
      return lodash.assign({}, state, {
        list: action.list,
        selected: {},
        total: action.total,
    ...
    case Constants.PICKUP_ORDERS_SET_SELECTED: {
