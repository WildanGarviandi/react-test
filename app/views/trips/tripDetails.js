...
import {TripDetailsFetch} from '../../modules/trips/actions/tripsFetch';
import {Page} from '../base';
import Accordion from '../base/accordion';
import styles from './styles.css';

const tripAttrs = ['driver', 'container', 'status', 'pickupTime', 'dropoffTime'];
const tripLabel = {
  container: 'Container',
  driver: 'Driver',
...
const routeAttrs = ['pickupAddress', 'pickupTime', 'dropoffAddress', 'dropoffTime', 'distance', 'etaDelivery', 'deliveryFee', 'netMargin', 'status', 'fleetManager'];
const routeLabel = {
...
function ProcessRoute(route) {
  ...
  const fleetManagerName = `${fleetManager.FirstName} ${fleetManager.LastName} / ${fleetManager.Email} / ${fleetManager.PhoneNumber}`;
...
const Route = React.createClass({
  render() {
    ...
    const routeTitle = `Route ${idx}: ${order.UserOrderNumber} / ${order.WebOrderID}`;
...
function StateToProps(state, ownProps) {
  ...
  const isFetchingTripDetails = tripsStore.state.isFetching;
  const trip = tripsStore.list[tripID] || {};
